...
int sobel(uchar *data, uchar *out, int width, int height) {
    ...
    for(int x = 0; x < width; x++) {
        for(int i = -1; i <= 1; i++) {
            ...
            sumX += v * G[ i + 1] / 2; // applique le masque sur le canal rouge
        }
    }
    for(int y = 0; y < height; y++) {
        for(int i = -1; i <= 1; i++) {
            float v = getValueAt(data, width, height, x, y + i);
            sumY += v * G[ i + 1] / 2; // applique le masque sur le canal rouge
        }
    }
    sum = (sumX + sumY) / 2;
