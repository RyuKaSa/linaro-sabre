...
// main_5_inlining.cpp
//
// Iteration 5: Kernel Separation with Loop Unrolling, Multi-threading, SIMD,
// and Function Inlining with Memory Access Optimization / Data Alignment.
// Compilation :
// >> g++ `pkg-config opencv --cflags` main_5_inlining.cpp -o main_5_inlining -lpthread `pkg-config opencv --libs`
...
// Inline grayscale conversion.
inline int compute_rgb_to_grayscale(uchar *data_in, uchar *data_out, int step_in, int step_out, int width, int height, int channels) {
    int i, j;
    for(i = 0; i < height; i++)
        for(j = 0; j < width; j++)
...
// Inline median helper.
inline char median_step(uchar *data, int width, int height, int i, int j) {
    char values[9];
...
inline int median_filter(uchar *data_in, uchar *data_out, int width, int height, int step) {
    int i, j;
    for(i = 0; i < height; i++)
        for(j = 0; j < width; j++)
...
// Inline SIMD helper: absolute value for 32-bit integers.
inline __m128i simd_abs_epi32(__m128i x) {
...
// Thread function for final convolution using SIMD with aligned loads.
void* finalConvPass(void *arg) {
    ...
    for(y = args->start; y < args->end; y++) {
        ...
        for(; x <= args->width - 5; x += 4) {
            __m128i top = _mm_load_si128((__m128i*)&args->Gx_temp[(y - 1) * args->width + x]);
            __m128i mid = _mm_load_si128((__m128i*)&args->Gx_temp[y * args->width + x]);
            __m128i bot = _mm_load_si128((__m128i*)&args->Gx_temp[(y + 1) * args->width + x]);
            ...
            __m128i gy_mid   = _mm_load_si128((__m128i*)&args->Gy_temp[y * args->width + x]);
...
int sobel_separable(uchar *data, uchar *out, int width, int height) {
    int *Gx_temp;
    int *Gy_temp;
    // Allocate 16-byte aligned memory for temporary buffers.
    if(posix_memalign((void**)&Gx_temp, 16, width * height * sizeof(int)) != 0) {
        printf("Allocation failed for Gx_temp\n");
        return -1;
    }
    if(posix_memalign((void**)&Gy_temp, 16, width * height * sizeof(int)) != 0) {
        printf("Allocation failed for Gy_temp\n");
        free(Gx_temp);
        return -1;
    }
