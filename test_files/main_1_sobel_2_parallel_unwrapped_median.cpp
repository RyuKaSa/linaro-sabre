...
char median_step(uchar *data, int width, int height, int x, int y) {
    ...
    // for(int index = 0; index < 9; index++) {
    //     values[index] = data[(y-1)*width+x-1]; 
    //     values[index] = data[(y-1)*width+x]; 
    //     values[index] = data[(y-1)*width+x+1]; 
    //     values[index] = data[y*width+x-1]; 
