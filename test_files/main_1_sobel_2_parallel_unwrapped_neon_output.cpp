...
int main() {
    ...
    int step_gray = Image_SEUIL->widthStep / sizeof(uchar);
    ...
    uchar *Data_out    = (uchar *)malloc(width * height);
    uchar *Data_median = (uchar *)malloc(width * height);
    uchar *Data_sobel  = (uchar *)malloc(width * height);

    for(frame = 0; (key != 'q' && key != 'Q') && frame < MAX_FRAMES; frame++) {
    ...
    printf("total time for %d frames : %lf (%lffps)\n", frame, total_time, frame/total_time);
