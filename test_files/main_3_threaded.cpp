...
// main_3_threaded.cpp
//
// Iteration 3: Kernel Separation with Loop Unrolling and Multi-threading using POSIX Threads
//
// Compilation :
// >> g++ `pkg-config opencv --cflags` main_3_threaded.cpp -o main_3_threaded -lpthread `pkg-config opencv --libs`
...
// Structure for horizontal differences computation.
typedef struct {
    int start;
    int end;
    int width;
...
// Structure for vertical differences computation.
typedef struct {
    int start;
    int end;
    int width;
...
// Structure for final convolution.
typedef struct {
    int start;
    int end;
    int width;
...
// Thread function for final convolution with loop unrolling.
void* finalConvPass(void *arg) {
    ...
    for(y = args->start; y < args->end; y++) {
        ...
        for(; x <= args->width - 5; x += 4) {
            int Gx0 = args->Gx_temp[(y - 1) * args->width + x]     + 2 * args->Gx_temp[y * args->width + x]     + args->Gx_temp[(y + 1) * args->width + x];
            int Gx1 = args->Gx_temp[(y - 1) * args->width + x + 1] + 2 * args->Gx_temp[y * args->width + x + 1] + args->Gx_temp[(y + 1) * args->width + x + 1];
            int Gx2 = args->Gx_temp[(y - 1) * args->width + x + 2] + 2 * args->Gx_temp[y * args->width + x + 2] + args->Gx_temp[(y + 1) * args->width + x + 2];
            int Gx3 = args->Gx_temp[(y - 1) * args->width + x + 3] + 2 * args->Gx_temp[y * args->width + x + 3] + args->Gx_temp[(y + 1) * args->width + x + 3];
            
            int Gy0 = args->Gy_temp[y * args->width + (x - 1)] + 2 * args->Gy_temp[y * args->width + x]     + args->Gy_temp[y * args->width + (x + 1)];
            int Gy1 = args->Gy_temp[y * args->width + x]     + 2 * args->Gy_temp[y * args->width + x + 1] + args->Gy_temp[y * args->width + (x + 2)];
            int Gy2 = args->Gy_temp[y * args->width + (x + 1)] + 2 * args->Gy_temp[y * args->width + x + 2] + args->Gy_temp[y * args->width + (x + 3)];
            int Gy3 = args->Gy_temp[y * args->width + (x + 2)] + 2 * args->Gy_temp[y * args->width + x + 3] + args->Gy_temp[y * args->width + (x + 4)];
            ...
            args->out[y * args->width + x]     = (uchar)(sum0 > 255 ? 255 : sum0);
            args->out[y * args->width + x + 1] = (uchar)(sum1 > 255 ? 255 : sum1);
            args->out[y * args->width + x + 2] = (uchar)(sum2 > 255 ? 255 : sum2);
            args->out[y * args->width + x + 3] = (uchar)(sum3 > 255 ? 255 : sum3);
        }
        ...
        for(; x < args->width - 1; x++) {
            ...
            args->out[y * args->width + x] = (uchar)(sum > 255 ? 255 : sum);
        }
    }
    pthread_exit(NULL);
}
